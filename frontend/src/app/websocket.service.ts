...
export class WebsocketService {
  ...
  connect(): void {
    this.socket.subscribe({
      ...
      error: err => {
        console.log('websocket service error: ', err);
        this.receiveMessage({meta: "disconnected", lobby: '', content: null});
  ...
  sendMessage(meta: any, lobby: string, content: any, user?: string, nickname?: string, state?: number): void {
    this.socket.next({
      meta: meta,
      lobby: lobby,
      user: user,
